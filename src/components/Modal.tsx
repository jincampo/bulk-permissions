...
interface ModalProps {
  ...
  embedded?: boolean; // when true, renders without outer overlay
}
...
const Modal: React.FC<ModalProps> = ({ 
  ...
  const modalBody = (
    <div className="bg-white rounded border border-[#eaecf1] w-full max-w-[600px] mx-4">
        {/* Modal Header */}
            ...
            <div>
              ...
              <div className="relative" ref={actionDropdownRef}>
                <button
                  ...
                  className={`w-full border border-gray-300 rounded-md px-3 py-2 text-sm bg-white text-left focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 flex justify-between items-center ${selectedAction ? 'text-gray-900' : 'text-gray-500'}`}
                >
                  <span>
                    {selectedAction ? actionOptions.find(opt => opt.value === selectedAction)?.title : 'Select your changes'}
                  </span>
                  <ChevronDownIcon />
                </button>
                
                {isActionDropdownOpen && (
                  <div className="absolute z-10 mt-1 w-full bg-white border border-[#eaecf1] rounded-[3px] shadow-[0px_4px_4px_0px_rgba(0,0,0,0.17)]">
                    <div className="py-2">
                      {actionOptions.map((option) => (
                        ...
                        >
                          <div className="flex-1">
                            <div className="text-[#2a2c35] text-[14.222px] leading-[1.5] font-normal">
                              {option.title}
                            </div>
                            <div className="text-[#6a6c75] text-[12.642px] leading-[1.5] mt-0.5">
                              {option.description}
                            </div>
                          </div>
                        </button>
                ...
                {selectedAction === 'add-apps' ? (
                  <>
                    ...
                    <div className="mb-5 relative">
                      <label className="block text-sm font-semibold text-[#2a2c35] mb-2">
                  ...
                  <>
                    ...
                    {/* Apps to apply - always show but disable when no app roles selected */}
                    <div className="mb-5 relative">
                      <label className="block text-sm font-semibold text-[#2a2c35] mb-2">
                      ...
                      <div className="relative" ref={appsDropdownRef}>
                        <button
                          type="button"
                          onClick={() => appRoles.length > 0 && setIsAppsDropdownOpen(!isAppsDropdownOpen)}
                          disabled={appRoles.length === 0}
                          className={`w-full border rounded-md px-3 py-2 text-sm text-left focus:outline-none flex justify-between items-start min-h-[40px] ${
                            appRoles.length === 0 
                              ? 'bg-gray-50 border-gray-200 text-gray-400 cursor-not-allowed' 
                              : 'bg-white border-gray-300 hover:border-blue-500 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 cursor-pointer'
                          }`}
                        >
                             <div className="flex-1 min-w-0">
                               {selectedApps.length > 0 ? (
                                 ...
                                 <span className={appRoles.length === 0 ? "text-gray-400" : "text-gray-500"}>
                                   {appRoles.length === 0 ? "Select app level roles first" : "Select apps to apply roles to"}
                                 </span>
                               )}
                             </div>
                             <svg className="w-4 h-4 text-gray-400 mt-1 ml-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                           ...
                           {isAppsDropdownOpen && appRoles.length > 0 && (
                             <div className="absolute z-10 mt-1 w-full bg-white border border-gray-300 rounded-md shadow-lg">
                               {/* Search input */}
                               <div className="p-3 border-b border-gray-200">
                                 <div className="relative">
        ...
        {/* Modal Footer */}
        <div className="flex items-center justify-between px-4 py-4 border-t border-[#dadce5]">
          <button
            onClick={handleClose}
